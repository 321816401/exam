...
<div class="main">
    ...
    <s:action name="Questions.view" id="view" executeResult="false">
        <s:param name="questions.id">${param.id}</s:param>
    </s:action>
    <form class="form">
        <input type="hidden" name="questions.id" value="${param.id}">
        <input type="hidden" name="questions.addTime" value="${view.questions.addTime}">
        <div><span>名称</span><input type="text" name="questions.name" value="${view.questions.name}"></div>
...
<script>
    function update(){
        $.ajax({
            url:'/ssh/Questions.update.action?'+$("form").serialize(),
            type:'POST',
            dataType:'json',
